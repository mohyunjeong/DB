...
-- SELECT 테이블1.컬럼명, 테이블2.컬럼명 -> 어느 테이블에 있는 컬럼명을 명시해서 출력해주겠다(경로 설정)
-- FROM 테이블1, 테이블2 -> 테이블1과 테이블2를 조인하겠다!!
-- WHERE 테이블1.컬럼명 = 테이블2.컬럼명 -> JOIN 조건을 적어주는 것

-- 수강생 정보 테이블과 성적표 테이블에서 학생ID, 이름, 과목, 성적 순으로 출력

-- 1. 조인할 대상 테이블의 정보를 확인
SELECT * FROM 수강생정보;
SELECT * FROM 성적표;

-- 2. FROM절에 조인할 테이블을 ,(콤마) 기준으로 작성(FROM)
-- 3. 조인 조건이 되는 특정 컬럼을 확인, WHERE 조건절에 조인 조건을 작성(WHERE)
--  > 조인 조건이 되는 특정 컬럼 : 조인할 테이블 간의 같은 결과 값을 가지는 컬럼!!
-- 4. SELECT 절에 해당되는 컬럼을 .(경로)를 설정하여 작성(SELECT)
SELECT 수강생정보.학생ID, 성적표.학생ID, 성적표.과목, 성적표.성적
FROM 수강생정보, 성적표
...
--  > 조인 조건이 되는 특정 컬럼 : 조인할 테이블간의 같은 결과 값(의미)을 가지는 컬럼
...
-- 별칭을 사용
-- 직원 테이블의 별칭을 'E', 부서 테이블의 별칭을 'D'를 사용해서 작성
...
-- 조인 조건 컬럼은 대부분 PK와 FK 관계로 이루어진다. 하지만!! 다 그런 것은 아니다!!

-- 실습) 직원ID가 100인 직원의 FIRST_NAME과 부서 이름을 출력
-- 직원 테이블을 별칭 E로 부서 테이블의 별칭을 D로 작성
...
-- ANSI 조인 문법 : 미국표준협회에서 만든 모든 DBMS에서 사용 가능한 문법

-- INNER JOIN : 내부 조인이라고 하며 조인 조건에서 동일한 값이 있는 행만 반환
-- > INNER는 생략 가능
...
-- 문제) 직원 테이블의 직원ID와 부서ID
--       부서 테이블에 있는 부서 이름을 출력 (CROSS JOIN)
SELECT E.EMPLOYEE_ID, E.DEPARTMENT_ID, D.DEPARTMENT_NAME
FROM EMPLOYEES E CROSS JOIN DEPARTMENTS D;


-- OUTER JOIN : 외부조인이라고 하며, 두 개의 테이블 간의 교집합을 조회하고,
-- 한쪽 테이블에만 있는 테이터도 포함시켜 조회하고 싶을 때 사용하는 조인 문법
-- > 한 쪽 데이터의 NULL 값도 출력하고 싶을 때 사용한다!!

-- LEFT OUTER JOIN : 왼쪽 테이블을 기준으로 NULL 값도 포함하고 싶을 때 사용
-- > 오라클 문법 조인 시 : 조인 조건에 반대인 오른쪽 컬럼에 (+) 기호를 작성

-- RIGHT OUTER JOIN : 오른쪽 테이블을 기준으로 NULL 값도 포함하여 출력
-- > 오라클 문법 조인 시 : 조인 조건에 반대인 왼쪽 컬럼에 (+) 기호를 작성

-- FULL OUTER JOIN : 양쪽에 NULL 값도 포함하여 출력
-- > 오라클 문법에서는 지원을 하지 않음, UNION 연산자 출력하는 방법이 있따.

-- 문제) 각 부서의 매니저 직원들의 부서 이름, 매니저 아이디, FIRST_NAME을 출력(INNER JOIN)
SELECT * FROM EMPLOYEES;
SELECT * FROM DEPARTMENTS;
